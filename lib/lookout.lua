...
function lookout:create(newLayerData, args)
	...
	function look:addLayerData(data)
		if not data then
			error("addLayerData requires a table of layers, or single layer")
		end
		if type(data) ~= "table" then
			error("addLayerData parameter must be a table")
		end
		if next(data) == nil then
			error("addLayerData parameter cannot be an empty table")
		end
		if not data[1] then
			data = { data }
		end -- parameter is a single layer
		if not self.layerData then
			self.layerData = {}
		end
		for i, ld in ipairs(data) do
			table.insert(self.layerData, ld)
		end
	end

	function look:newLayer(img, args)
		if not img then
			error("newLayer requires an image")
		end
		...
		function layer:update(dt) -- calculate parallax positioning
			if layer.anim and type(layer.anim.update) == "function" then
				layer.anim:update(dt)
			end

			if layer.look.spoofX ~= 0 then
				layer.spoofX = layer.spoofX + layer.look.spoofX * dt
			end
			if layer.look.spoofY ~= 0 then
				layer.spoofY = layer.spoofY + layer.look.spoofY * dt
			end
			...
			if not layer.fixedX then
				layer.relX = layer.relX - (diffX / layer.depth)
			end
			if not layer.fixedY then
				layer.relY = layer.relY - (diffY / layer.depth)
			end

			if layer.relX < -1 * layer.width then
				layer.relX = layer.relX + layer.width
			end
			if layer.relX > layer.width then
				layer.relX = layer.relX - layer.width
			end

			if layer.relY < -1 * layer.height then
				layer.relY = layer.relY + layer.height
			end
			if layer.relY > layer.height then
				layer.relY = layer.relY - layer.height
			end
